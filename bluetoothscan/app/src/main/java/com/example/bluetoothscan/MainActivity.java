...
public class MainActivity extends AppCompatActivity implements AdapterView.OnItemClickListener{
    ...
    public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
        ...
        if(Build.VERSION.SDK_INT > Build.VERSION_CODES.JELLY_BEAN_MR2){
            ...
            ConnectThread mconnect = new ConnectThread(mBTDevices.get(i), myUUID);
            myBTsocket = mconnect.getbTSocket();
            Log.d(TAG, "onItemClick: " + mBTDevices.get(i));


            mconnect.start();
            new Thread(new Runable2(mconnect, mTrans, mHandler));
        }
    }
    ...
    private void listenerHanler (){
            Log.d(TAG, "run: recive massage");
